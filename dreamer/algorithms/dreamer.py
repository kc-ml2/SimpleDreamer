...
from dreamer.utils.utils import pixel_normalization, compute_lambda_values, create_normal_dist, DynamicInfos
from dreamer.utils.buffer import ReplayBuffer

class Dreamer:
    def __init__(self, observation_shape, discrete_action_bool, action_size, writer, device, config):
        self.device = device
    ...
    def dynamic_learning(self, data):
        ...
        for t in range(1, self.config.batch_length): 
            ...
            self.dynamic_learning_infos.append(priors = prior,\
    ...
    def _model_update(self, data, posterior_info):
        ...
        reconstruction_observation_loss = reconstructed_observation_dist.log_prob(pixel_normalization(data.observation[:,1:]))
        if self.config.use_continue_flag:
        ...
        model_loss.backward()
        nn.utils.clip_grad_norm_(self.model_params, self.config.clip_grad, norm_type=self.config.grad_norm_type)
    ...
    def _agent_update(self, behavior_learning_infos):
        ...
        if self.config.use_continue_flag:
            continues = self.continue_predictor(behavior_learning_infos.priors, behavior_learning_infos.deterministics).mean
        else :
            continues = self.config.discount * torch.ones_like(values)
        
        lambda_values = compute_lambda_values(predicted_rewards, values, continues, self.config.horizon_length, self.device, self.config.lambda_)
        
        actor_loss = - torch.mean(lambda_values)

        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        nn.utils.clip_grad_norm_(self.actor.parameters(), self.config.clip_grad, norm_type=self.config.grad_norm_type)
        self.actor_optimizer.step()
        
        value_dist = self.critic(behavior_learning_infos.priors.detach()[:,:-1], behavior_learning_infos.deterministics.detach()[:,:-1])
        value_loss = - torch.mean(value_dist.log_prob(lambda_values.detach()))

        self.critic_optimizer.zero_grad()
        value_loss.backward()
        nn.utils.clip_grad_norm_(self.critic.parameters(), self.config.clip_grad, norm_type=self.config.grad_norm_type)
    ...
    def environment_interaction(self, env, num_interaction_episodes, train = True):        
            ...
            while not done: 
                ...
                if done:
                    if train :
                        ...
                        self.writer.add_scalar('training score',
                                    score,
        ...
        if not train :
            ...
            self.writer.add_scalar('test score',
                                    evaluate_score,
                                    self.num_total_episode)
