...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size, chunk_size):
        ...
        sample_index = np.random.randint(0,
                                 self.capacity if self.full else last_filled_index,
        ...
        sample = AttrDict({'observation': observation, 'action' : action, 'reward' : reward, 'next_observation' : next_observation, 'done' : done})
        return sample